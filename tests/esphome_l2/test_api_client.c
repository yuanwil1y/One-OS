...
static bool recv_frame(int fd,uint16_t*t,uint8_t*p,size_t cap,size_t*n){uint8_t z;if(!allr(fd,&z,1)||z!=0){fprintf(stderr,"P: bad preamble %02x\n",z);fflush(stderr);return false;}uint32_t len,type;if(!readv(fd,&len)||!readv(fd,&type)||len>cap){fprintf(stderr,"P: bad header\n");fflush(stderr);return false;}if(len&&!allr(fd,p,len))return false;fprintf(stderr,"P: read frame type=%u len=%u\n",(unsigned)type,(unsigned)len);fflush(stderr);*t=(uint16_t)type;*n=len;return true;}static bool send_frame(int fd,uint16_t t,const uint8_t*p,size_t n){uint8_t h[12];size_t k=0;h[k++]=0;k+=encv((uint32_t)n,h+k);k+=encv(t,h+k);fprintf(stderr,"P: write frame type=%u len=%u\n",(unsigned)t,(unsigned)n);fflush(stderr);return allw(fd,h,k)&&(!n||allw(fd,p,n));}
