...
{uint8_t fh[3];if(!allr(fd,fh,3)){}else{fr[0]=fh[0];fr[1]=fh[1];fr[2]=fh[2];}assert(fr[0]==0x01);}n=((size_t)fr[1]<<8)|fr[2];assert(n<=sizeof(fr)&&allr(fd,fr,n));
...
{uint8_t fh[3];if(!allr(fd,fh,3)){}else{fr[0]=fh[0];fr[1]=fh[1];fr[2]=fh[2];}assert(fr[0]==0x01);}n=((size_t)fr[1]<<8)|fr[2];assert(n<=sizeof(fr)&&allr(fd,fr,n));assert(ntr_open(&st,fr,n,&t,pl,sizeof(pl),&n)&&t==9);
