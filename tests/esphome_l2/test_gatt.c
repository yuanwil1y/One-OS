...
typedef struct{void*owner;esphome_ble_gatt_backend_notify_fn notify;esphome_ble_gatt_session_t *session;bool connected;bool fail_connect;bool lose_next;bool timeout_next;bool cancel_during_read;bool cancel_during_discover;uint16_t last_cccd;bool notify_enabled;int disconnects,cancels,reads;} mock_t;
...
static esp_err_t mr(void*ctx,uint16_t h,uint8_t*out,size_t cap,size_t*len,uint32_t t,int*n){(void)h;(void)t;mock_t*c=ctx;c->reads++;if(c->cancel_during_read){/* Another task cancels while this read is outstanding. */assert(esphome_ble_gatt_cancel(c->session)==ESP_OK);*len=0;return ESP_OK;}if(c->timeout_next){c->timeout_next=false;*n=99;return ESP_ERR_TIMEOUT;}if(c->lose_next){c->lose_next=false;c->connected=false;*n=8;return ESP_FAIL;}static const uint8_t v[]={1,2,3};*len=sizeof(v);if(cap<sizeof(v))return ESP_ERR_INVALID_SIZE;memcpy(out,v,sizeof(v));return ESP_OK;}
