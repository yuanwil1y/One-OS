...
#define SD_TEST_PAYLOAD "One-OS v0.1.0-beta.2 smoke test\n"
...
static esp_err_t run_wifi(smoke_test_result_t *result)
{
    ...
    if (err == ESP_OK) {
        ...
        (void)snprintf(result->detail, sizeof(result->detail), "%u APs found", (unsigned)ap_count);
    }
...
static void ble_on_sync(void)
{
    ...
    if (rc == 0) rc = ble_hs_id_infer_auto(0, &s_ble_own_addr_type);
    s_ble_sync_ok = rc == 0;
    if (s_ble_sync_sem != NULL) xSemaphoreGive(s_ble_sync_sem);
}
...
static int ble_gap_event(struct ble_gap_event *event, void *arg)
{
    ...
    if (event->type == BLE_GAP_EVENT_DISC) {
        ...
        if (s_ble_done_sem != NULL) xSemaphoreGive(s_ble_done_sem);
    }
    return 0;
}
...
static esp_err_t run_ble(smoke_test_result_t *result)
{
    ...
    if (s_ble_sync_sem == NULL || s_ble_done_sem == NULL) {
        if (s_ble_sync_sem != NULL) vSemaphoreDelete(s_ble_sync_sem);
        if (s_ble_done_sem != NULL) vSemaphoreDelete(s_ble_done_sem);
        s_ble_sync_sem = NULL;
        s_ble_done_sem = NULL;
    ...
    if (err != ESP_OK) goto cleanup_sems;
    ...
    params.limited = 0;
    rc = ble_gap_disc(s_ble_own_addr_type, (int32_t)BLE_SCAN_MS,
    ...
    (void)snprintf(result->detail, sizeof(result->detail),
                   "%lu advertising reports", (unsigned long)s_ble_report_count);
    err = ESP_OK;

stop_host: {
        int stop_rc = nimble_port_stop();
        if (err == ESP_OK && stop_rc != 0) err = ESP_FAIL;
        int deinit_rc = nimble_port_deinit();
        if (err == ESP_OK && deinit_rc != ESP_OK) err = deinit_rc;
    }
...
{
    if (s_i154_callback_registered) return ESP_OK;
    ...
    if (err == ESP_OK) s_i154_callback_registered = true;
    return err;
}
...
static esp_err_t run_i154(smoke_test_result_t *result)
{
    ...
    for (uint8_t channel = 11; channel <= 26; ++channel) {
        ...
        if (err == ESP_OK) {
            ...
            if (err == ESP_OK) promiscuous = true;
        }
        if (err == ESP_OK) {
        ...
        if (promiscuous) {
            esp_err_t e = esp_ieee802154_set_promiscuous(false);
            if (err == ESP_OK && e != ESP_OK) err = e;
        }
        if (receiving) {
            esp_err_t e = esp_ieee802154_sleep();
            if (err == ESP_OK && e != ESP_OK) err = e;
        }
        if (enabled) {
            esp_err_t e = esp_ieee802154_disable();
            if (err == ESP_OK && e != ESP_OK) err = e;
        }
        if (err != ESP_OK) break;
        ++channels_completed;
...
static esp_err_t run_sd_rw(smoke_test_result_t *result)
{
    ...
    if (err == ESP_OK && unmount_err != ESP_OK) err = unmount_err;
    if (err == ESP_OK) {
        (void)snprintf(result->detail, sizeof(result->detail), "write/read byte-verified");
    }
    return err;
}
...
static esp_err_t run_sd_format(smoke_test_result_t *result)
{
    esp_err_t err = board_sd_mount(false);
    if (err == ESP_OK) {
        sdmmc_card_t *card = board_sd_card();
        err = card != NULL
                  ? esp_vfs_fat_sdcard_format(BOARD_SD_MOUNT_POINT, card)
                  : ESP_ERR_INVALID_STATE;
    } else {
        /* User explicitly confirmed destructive formatting. */
        err = board_sd_mount(true);
    }

    if (err == ESP_OK) err = sd_rw_on_mounted();
...
esp_err_t smoke_test_run(smoke_test_kind_t kind, smoke_test_result_t *out_result)
{
    ...
    esp_err_t err;
    switch (kind) {
    case SMOKE_TEST_WIFI: err = run_wifi(out_result); break;
    case SMOKE_TEST_BLE: err = run_ble(out_result); break;
    case SMOKE_TEST_I154: err = run_i154(out_result); break;
    case SMOKE_TEST_SD_RW: err = run_sd_rw(out_result); break;
    case SMOKE_TEST_SD_FORMAT: err = run_sd_format(out_result); break;
