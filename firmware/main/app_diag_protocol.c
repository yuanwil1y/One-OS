...
const char *app_diag_command_name(app_diag_command_t command)
{
    ...
    case APP_DIAG_CMD_PORTAL:    return "portal";
...
static app_diag_command_t parse_command(const char *text)
{
    ...
    if (text_equal_ci(text, "portal")) {
        return APP_DIAG_CMD_PORTAL;
    }
...
app_diag_error_t app_diag_parse(const char *line, app_diag_request_t *out)
{
    ...
    case APP_DIAG_CMD_PORTAL: {
         ...
         *
         * The verb travels in `action`, because that is the field the runtime already has
         * for "what to do". No operand carries a secret in either direction: the password
         * is generated by the firmware and only ever printed to this console.
         *
         * The length check and the copy are written the same way the other commands in
         * this switch do it, and NOT with a helper: this file is the diagnostic protocol
         * layer, and keeping its one bounded-copy idiom is what makes it auditable in one
         * reading.
         */
        if (count != 4u || extra > 0u) {
            return APP_DIAG_ERR_BAD_REQUEST;
        }
        if (strlen(tokens[3]) >= APP_DIAG_TOKEN_LEN) {
            return APP_DIAG_ERR_INVALID_ARGUMENT;
        }
        memcpy(out->action, tokens[3], strlen(tokens[3]) + 1u);
        break;
    }
