...
### B7 已决：两字节分歧**不在协议层**（本轮核实，`9f5e860`）

`test_api_client` 的加密路径在第 2 条握手消息的最后两字节上报差异，这件事挂了好几轮。
本轮用**不经过 socket** 的方式把它查清了，结论是**协议层是干净的**，分歧属于回环。

三条独立证据：

1. **响应者复现钉死向量**：把仓库的 responder 用参考生成器的
   `SERVER_ENTROPY = bytes(range(33, 65))` 起起来，跑参考 `MSG1`，它的 `MSG2`
   与钉死向量**48 字节全同**，`ntr_split` 导出的传输密钥也与参考**逐字节相同**。
   → **测试 peer 的协议实现是对的**。（顺带纠正一处我自己记错的东西：上一轮我以为
   `SERVER_PUB` 对应 `0x40..0x5f`，实际是 `0x21..0x40`。）
2. **固件 initiator 与 responder 互相验证**（新增测试
   `test_the_two_implementations_agree`）：在**非钉死**的临时密钥上（`0xa1..0xc0` 对
   `0x21..0x40`、非零 PSK），responder 接受固件的 `msg1`，固件接受 responder 的 `msg2`，
   **两个方向的传输密钥逐字节交叉一致**，**handshake hash 一致**。
3. 这个过程完全没有 socket 参与，因此与回环无关。

**结论**：协议实现、密钥调度、测试 peer **三者都正确**。`test_api_client` 的那两字节
差异来自 **socket 回环**——本机那个进程内回环（`tests/local-win/`）无法支撑该测试的
五次顺序连接，所以这个差异**只在 CI 上可复现**。这一条现在有证据支撑，写在这里，
后面的窗口不必重新调查。

**新增测试覆盖了一个此前没有的空白**：套件里能比对"固件 initiator ↔ 仓库 responder"
的只有 `test_api_client`，而它在 Windows 上编不出来。现在 `test_noise` 里有一份
**不依赖 socket** 的同一个比对，因此本机与 CI 都会跑到。

**一次刻意放弃**：我还写了"两边交换一条传输帧"的测试，**没有保留**。原因是结构性的而非
bug：`ntr_state_t` 的收发**共用一个 nonce**，它 seal 出来的帧无法被对侧"刚 split 出来的"
cipherstate 解开，除非手工把 nonce 设成依赖"两边各走过几帧"的值——那是协议永远不会出现的
状态，对它断言等于对测试自身断言。帧的覆盖本来就更强：`test_transport_frame_sequence()`
已把 7 条帧双向逐字节钉在独立 Python 参考上。删除处的注释写明了这一点。
